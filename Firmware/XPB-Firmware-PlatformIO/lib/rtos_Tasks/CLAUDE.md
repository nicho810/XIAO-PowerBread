...
serialTask.h/cpp: 双模串口任务 (优先级1)，5ms 基础循环，IDLE 时文本调试 (human/plotter，可配置间隔)，收到 START 自动切 XPB Binary Protocol v1.2 二进制流 (100Hz)，依赖 xpb_protocol 模块
...
serialPrintTask ──(Serial RX)──→ XpbSession 状态机 → 二进制协议流 (Serial TX)
```
